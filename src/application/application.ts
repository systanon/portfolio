...
import type { SignInDto, SignUpDto, UserProfile, UserProfileUpdateInfo } from '@/types/auth';
...
> {
  ...
  public async getProfile(): Promise<UserProfile | AppError> {
    ...
    this.profileLoading = new Promise<void>((resolve) => (this.resolveProfileLoading = resolve));
  ...
  public async updateProfileInfo(dto: UserProfileUpdateInfo): Promise<AppError | string> {
